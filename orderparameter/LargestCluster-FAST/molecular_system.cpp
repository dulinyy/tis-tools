...
void CMolecularSystem::get_AllNeighbors()
{
	...
	for (int ti=0; ti<(nop-1); ti++)
	{
		for (int tj=ti+1; tj<nop; tj++)
		{
			...
			if (d < nd) {
				this->molecules[ti].neighbors[ccount[ti]] = tj; 
				this->molecules[ti].neighbordist[ccount[ti]] = d; 
				ccount[ti] += 1;   

				this->molecules[tj].neighbors[ccount[tj]] = ti;
				this->molecules[tj].neighbordist[ccount[tj]] = d;
				ccount[tj] +=1;
			}
		}
	}
	for (int ti=0; ti<nop; ti++){
		this->molecules[ti].n_neighbors = ccount[ti];
	}
}

void CMolecularSystem::get_AllNeighborsAndDistances()
{
	...
	for (int ti=0; ti<(nop-1); ti++)
	{
		for (int tj=ti+1; tj<nop; tj++)
		{
			...
			if (d < nd) {
				this->molecules[ti].neighbors[ccount[ti]] = tj; 
				this->molecules[ti].neighbordist[ccount[ti]] = d; 
				this->molecules[ti].n_diffx[ccount[ti]] = diffx;
				this->molecules[ti].n_diffy[ccount[ti]] = diffy;
				this->molecules[ti].n_diffz[ccount[ti]] = diffz;
				this->convert_SphericalCoordinates(diffx, diffy, diffz, r, phi, theta);
				this->molecules[ti].n_r[ccount[ti]] = r;
				this->molecules[ti].n_phi[ccount[ti]] = phi;
				this->molecules[ti].n_theta[ccount[ti]] = theta;
				ccount[ti] += 1;   

				this->molecules[tj].neighbors[ccount[tj]] = ti;
				this->molecules[tj].neighbordist[ccount[tj]] = d;
				this->molecules[tj].n_diffx[ccount[tj]] = -diffx;
				this->molecules[tj].n_diffy[ccount[tj]] = -diffy;
				this->molecules[tj].n_diffz[ccount[tj]] = -diffz;
				this->convert_SphericalCoordinates(-diffx, -diffy, -diffz, r, phi, theta);
				this->molecules[tj].n_r[ccount[tj]] = r;
				this->molecules[tj].n_phi[ccount[tj]] = phi;
				this->molecules[tj].n_theta[ccount[tj]] = theta;
				ccount[tj] +=1;
			}
		}
	}
...
void CMolecularSystem::QLM(int l,int m,int ti,int tj,double &realYLM, double &imgYLM )
{
	...
	realYLM = 0.0;
	imgYLM = 0.0;
	//JR: this should be done outside the function since it is the same each time!
	//JR: maybe even save the distances/spherical coordinates directly when getting the neighbours
...
void CMolecularSystem::QLM(int l,int m,double theta,double phi,double &realYLM, double &imgYLM )
{
	realYLM = 0.0;
	imgYLM = 0.0;
	if (m < 0) {
...
//Calculates the complex components of ONLY q6m vector
void CMolecularSystem::calculate_complexQLM_6()
{
	//nn = number of neighbors
	int nn,nop;
	double realti,imgti;
	...
	for (int ti= 0;ti<nop;ti++)
	{
		...
		for (int mi = -6;mi < 7;mi++)
		{
			...
			for (int ci = 0;ci<nn;ci++)
			{
				QLM(6,mi,this->molecules[ti].n_theta[ci],this->molecules[ti].n_phi[ci],realYLM, imgYLM);
				realti += realYLM;
				imgti += imgYLM;
			}
			realti = realti/(double(nn));
...
//JR: determines values for molecule[i].arealQ6[mi]/aimagQ6[mi]/arealQ4[mi]/aimagQ4[mi]
...
//Calculates local Q's from the q-vector
//JR: determines values for molecule[i].Q6/Q4
...
//determines values for molecule[i].AQ6/AQ4
...
//Calculates the Averaged Q6 and Averaged Q4 from the complex vector
//determins values for globalQ6 and globalQ4
...
void CMolecularSystem::calculate_largestClusterparameter_Full()
{
	int criterium;
	int greatestbelongsto;
	//Find all particles within a radius of neighbourdistancess
	this->get_AllNeighborsAndDistances();
	//Get Q6 values
	this->calculate_complexQLM_6();
	//and the number of bonds to find the largest cluster
	this->calculate_frenkelNumbers();
...
void CMolecularSystem::harvestCluster(const int ti, const int numberofCluster,const int criterium)
{
	...
	{    
		//numneigh=this->molecules[ti].n_neighbors;
		neighbor =  this->molecules[ti].neighbors[c];
		//cout<< ti <<"\t"<< c <<"\t" <<neighbor<<"\t"<< numberofCluster<< "\t" <<this->molecules[neighbor].belongsto<<"\t"<<l<<"\n";
	      
		if ((this->molecules[neighbor].belongsto == nilvalue) && (clusterCriterium(neighbor,criterium)))
		{       
			//lastsolidneigh=neighbor;
		...
		{   
			l+=1;
			// if (l == numneigh) cout<< ti <<"\t"<<this->molecules[ti].n_neighbors<<"\t"<<l<<"\n";
			// if (l == numneigh) goto getout;
		}
	}
	// getout:
	//return;	  
...
double CMolecularSystem::get_absDistance(int ti ,int tj,double &diffx ,double &diffy,double &diffz)
{
