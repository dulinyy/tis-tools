...
double CMolecularSystem::get_NumberFromBond(int ti,int tj,int id)
{
  ...
  //if (connection<0){ cout<<"negative conn"<<connection<<endl;}
  return connection;
}
...
void CMolecularSystem::calculate_frenkelNumbers()
{
  ...
  for (int ti= 0;ti<this->parameter->nop;ti++)
	...
	{
	  scalar = this->get_NumberFromBond(ti,this->molecules[ti].neighbors[c],this->molecules[ti].id);
	  if (scalar > 0.5) frenkelcons += 1;   
	  this->molecules[ti].avq6q6 += scalar;
  }
	
      this->molecules[ti].avq6q6 /= this->molecules[ti].n_neighbors;
      //cout <<this->molecules[ti].avq6q6<<"avq66"<<"\n";
...
int CMolecularSystem::get_greatestCluster(int criterium,int &greatestbelongsto)
{
  ...
  ofstream fout("cluster.dat");
  ofstream fout2("clusterlist.dat");
  ...
  for (int ti= 0;ti<nop;ti++)
     {
      ...
      if (this->molecules[ti].belongsto != nilvalue)
      {
     fout2<<molecules[ti].id<<"   "<<this->molecules[ti].belongsto<<endl;
       }
  }
  ...
  */
  ...
  //cout << setprecision(10) << greatestcluster <<"\t"<<fcccluster<<"\t"<<bcccluster<<"\t"<<hcpcluster<<"\t"<< nonscluster <<"\n";
...
void CMolecularSystem::readcrystalHistogramFiles()
{
  ...
  string bccfile= "/home/users/menonsqr/Documents/Thesis/Supporting_codes/crystalstruct_largestnuc_OV/probaq46_bcc20.dat";
  ...
  bcchistoFile.close();
   ...
   string fccfile= "/home/users/menonsqr/Documents/Thesis/Supporting_codes/crystalstruct_largestnuc_OV/probaq46_fcc20.dat";
   ...
   if (fcchistoFile.is_open())
  {
   ...
   string hcpfile= "/home/users/menonsqr/Documents/Thesis/Supporting_codes/crystalstruct_largestnuc_OV/probaq46_hcp20.dat";
...
void CMolecularSystem::readLiquidHistogramFile()
{
  ...
  //char dummy_char[256];                //dummy line
  ifstream histoFile;
  string filename= "/home/users/menonsqr/Documents/Thesis/Supporting_codes/crystalstruct_largestnuc_OV/probaq46_liq20.dat";
  //ifstream histoFile = ifstream(filename.c_str(),ifstream::in);
  histoFile.open(filename.c_str(),ifstream::in);
